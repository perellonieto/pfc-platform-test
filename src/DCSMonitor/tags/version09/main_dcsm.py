...
class DesignerMainWindow(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent = None):
        ...
        QtCore.QObject.connect(self.timer_data, QtCore.SIGNAL("timeout()"), self.calculate_data)
    ...
    def recive_data(self):
        ...
        while self.ser.inWaiting() > INPUT_DATA_SIZE+1:
            ...
            if data[20] == chr(1):
                ...
                if i[0] < STACK_SIZE:
                    for z in range(i[0]):
                        new_device = struct.unpack("I", data[(4*z)+25]+data[(4*z)+24]+data[(4*z)+23]+data[(4*z)+22])
                        new_string = str(new_device[0])
                        
                        #new_device = struct.pack("s", data[(4*z)+25]+data[(4*z)+24]+data[(4*z)+23]+data[(4*z)+22])
                        #new_string = binascii.hexlify(new_device)
                        print("buscant a la llista : "+new_string)
    ...
    def update_graph(self):
        ...
        if self.updated_data == 1:
            ...
            #self.mplWidget.canvas.ax.set_ylim(-YLIM, YLIM)
            self.mplWidget.canvas.ax.set_xlim(self.time-XLIM, self.time)
    ...
    def save_image(self):
        """Puts name and directory of the file to save"""
        # open the dialog and get the selected file
        #file_one = QtGui.QFileDialog.getSaveFileName(self,QtGui.QApplication.translate("MainWindow", "Save plot as PDF file", None, QtGui.QApplication.UnicodeUTF8), "", "Portable Network Graphics (*.png)")
        #file_one = QtGui.QFileDialog.getSaveFileName(self,QtGui.QApplication.translate("MainWindow", "Save plot as PDF file", None, QtGui.QApplication.UnicodeUTF8), "", "Portable Network Graphics (*.pdf)")
        ...
        if file_one:
            ...
            self.textBrowser.append(QtGui.QApplication.translate("MainWindow", "Saving to ", None, QtGui.QApplication.UnicodeUTF8)+file_one)

            plt.clf()
            lines = plt.plot(self.graf_t, self.graf_r, self.graf_t, self.graf_u, self.graf_t, self.graf_x1, self.graf_t, self.graf_x0)
            plt.axis([self.graf_t[0], self.graf_t[-1], -3, 3])
            plt.setp(lines[0], lw=1, label=str(QtGui.QApplication.translate("MainWindow", "Reference",  None, QtGui.QApplication.UnicodeUTF8)), color = 'blue')
            plt.setp(lines[1], lw=2, label=str(QtGui.QApplication.translate("MainWindow", "Input",      None, QtGui.QApplication.UnicodeUTF8)), color='red')
            plt.setp(lines[2], lw=1, label=str(QtGui.QApplication.translate("MainWindow", "First Int.",  None, QtGui.QApplication.UnicodeUTF8)), color='grey')
            plt.setp(lines[3], lw=2, label=str(QtGui.QApplication.translate("MainWindow", "Second Int.", None, QtGui.QApplication.UnicodeUTF8)), color='orange')
            #plt.setp(lines[4], lw=1, color='red', label="crit")
 
            
            plt.title(str(QtGui.QApplication.translate("MainWindow", "Double Integrator", None, QtGui.QApplication.UnicodeUTF8)))
            plt.suptitle(str(QtGui.QApplication.translate("MainWindow", "Networked and Embedded Control Systems:", None, QtGui.QApplication.UnicodeUTF8)))
            plt.xlabel(str(QtGui.QApplication.translate("MainWindow", "Timestamp", None, QtGui.QApplication.UnicodeUTF8)))
            plt.ylabel(str(QtGui.QApplication.translate("MainWindow", "Values", None, QtGui.QApplication.UnicodeUTF8)))
            ...
            try:
                ...
                self.textBrowser.append(QtGui.QApplication.translate("MainWindow", "Trying to save as .svg", None, QtGui.QApplication.UnicodeUTF8))
                filename = str(file_one) + '.svg'
                plt.savefig(filename, dpi=300)
                pass
    ...
    def toggled_monitor(self):
        ...
        if self.pushButton_monitor.isChecked():
            ...
            #bytes_id_link = struct.pack("B", id_link)
            
            #bytes_id_link = struct.pack("L", id_link)
            
            #bytes_id_link = array.array("B")
            
            #bytes_id_link.append(id_link&0xFF)
            #bytes_id_link.append((id_link>>8)&0xFF)
            #bytes_id_link.append((id_link>>16)&0xFF)
            #bytes_id_link.append((id_link>>24)&0xFF)
            # send Monitor
            #word = struct.pack("BBBBBBBB", ID_MONITOR,0,0,0,bytes_id_link[3],bytes_id_link[2],bytes_id_link[1],bytes_id_link[0])
            
            word = struct.pack("BBBBBBBB", ID_MONITOR,0,0,0,(id_link>>24)&0xFF,(id_link>>16)&0xFF,(id_link>>8)&0xFF,id_link&0xFF)
