...
void Send_Sensor2Supervisor_message(float *);//Function prototypes
...
TASK(TaskSensor_supervision)
{
	...
	Send_Sensor2Supervisor_message(&x[0]);//identifier=
}
...
int main(void)
{
	...
	SetRelAlarm(AlarmSensor,1000,50);//Sensor activates every 50ms
	SetRelAlarm(AlarmSupervision, 1000, 10);//Data is sent to the PC every 10ms
	SetRelAlarm(AlarmSensor_supervision, 1000, 10);//Data is sent to the supervisor 10ms
...
void Send_Sensor2Supervisor_message(float *data)
{
	...
	tx_ecan1message.id=ID_SUPERVISOR;//Identifier;
	tx_ecan1message.message_type=0;//0->Normal, 1->Remote Transmit
	tx_ecan1message.data_length=8;//Length of data (0 to 8 bytes)
	tx_ecan1message.data[0]= *p_data;
	tx_ecan1message.data[1]=*(p_data+1);
